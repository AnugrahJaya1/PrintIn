	  ...
	  if(isset($_POST['inquiry_reply']) && !empty($_POST['inquiry_reply'])) {
	  	...
	  	if( !defined( 'DOING_WCFM_EMAIL' ) ) 
			...
			if( $inquiry_customer_email ) {
				...
				if( $inquiry_vendor_id ) $enquiry_for_label = $WCFM->wcfm_vendor_support->wcfm_get_vendor_store_name_by_vendor( $inquiry_vendor_id ) . ' ' . apply_filters( 'wcfm_sold_by_label', $inquiry_vendor_id, __( 'Store', 'wc-frontend-manager' ) );
						...
						if( $is_allow_enquiry && apply_filters( 'wcfm_is_allow_enquiry_vendor_notification', true ) ) {
							...
							if( $vendor_email ) {
								if( apply_filters( 'wcfm_is_allow_enquiry_customer_reply', true ) && $WCFM->wcfm_vendor_support->wcfm_vendor_has_capability( $inquiry_vendor_id, 'view_email' ) ) {
									$vendor_reply = true;
									$headers[] = 'Reply-to: ' . $WCFM->wcfm_vendor_support->wcfm_get_vendor_store_name_by_vendor( $inquiry_vendor_id ) . ' ' . apply_filters( 'wcfm_sold_by_label', $inquiry_vendor_id, __( 'Store', 'wc-frontend-manager' ) ) . ' <' . $vendor_email . '>';
								}
							}
						}
					}
	  ...
	  if(isset($_POST['inquiry_reply']) && !empty($_POST['inquiry_reply'])) {
	  	...
	  	$wcfm_create_enquiry_reply = "INSERT into {$wpdb->prefix}wcfm_enquiries_response 
			...
			$reply_mail_body =   '<br/>' . __( 'Hi', 'wc-frontend-manager' ) .
													 ',<br/><br/>' . 
													 __( 'You have received reply for your "{enquiry_for}" inquiry. Please check below for the details: ', 'wc-frontend-manager' ) .
													 '<br/><br/><strong><i>' . 
													 '"{enquiry_reply}"' . 
													 '</i></strong><br/><br/>' .
