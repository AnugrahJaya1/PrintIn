  ...
  /**
	...
	public function show_star_rating( $store_rating = 0, $vendor_id = 0 ) {
		...
		if ( apply_filters( 'wcfm_is_pref_vendor_reviews', true ) ) { ?>
			<div class="wcfmmp-store-rating" title="<?php if( $store_rating ) { echo sprintf( __( 'Rated %s out of 5', 'wc-multivendor-marketplace' ), $store_rating ); } else { _e( 'No reviews yet!', 'wc-multivendor-marketplace' ); } ?>">
		...
		if( !empty( $vendor_reviews ) ) {
			foreach( $vendor_reviews as $vendor_review ) {
				...
				$wcfm_review_submit = "INSERT into {$wpdb->prefix}wcfm_marketplace_reviews 
													...
													VALUES
													({$vendor_id}, {$vendor_review->user_id}, '{$vendor_review->comment_author}', '{$vendor_review->comment_author_email}', '{$review_title}', '{$vendor_review->comment_content}', '{$review_rating}', {$approved})";
