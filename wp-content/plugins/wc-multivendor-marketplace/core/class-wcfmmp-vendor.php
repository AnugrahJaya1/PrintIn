				...
				if( !is_array( $meta->key ) ) {
					...
					if( $meta->key == '_vendor_id' ) {
						...
						if( $this->is_vendor_sold_by( absint($meta->value) ) ) {
							...
							$html = '<ul class="wc-item-meta"><li><strong class="wc-item-meta-label">' . $sold_by_text . ':</strong> ' . $WCFM->wcfm_vendor_support->wcfm_get_vendor_store_by_vendor( absint($meta->value) ) . '</li></ul>' . $html;
						}
					}
				}
			}
